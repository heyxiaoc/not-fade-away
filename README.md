...
- [**如何保证 Fable 不被偷换**](番外篇-Fable不被偷换.md) —— Fable 5 挂着一层「话题分类器」路由：碰到网安 / 生化 / 模型蒸馏话题，它会**静默**把你换成 Opus 4.8，还会赖着不走。这篇是真机实测：它怎么触发、怎么确认自己是不是被换了、以及那个 `switchModelsOnFlag` 设置怎么让它「当面报错、绝不偷换」。
