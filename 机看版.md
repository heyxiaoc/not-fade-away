...
> **底层基于 Claude Code 官方的 channels 功能**（消息注入 + `reply` 工具；官方文档 <https://code.claude.com/docs/en/channels>，自建 channel 参考 <https://code.claude.com/docs/en/channels-reference>）。本系统的 web 前端就是一个**自定义 channel**（research preview 期用 `--dangerously-load-development-channels` 加载）。好处是用官方 feature 实现同样的功能，且交互式常驻会话走订阅、不进 Agent SDK 计费池——**同样功能、成本更低**。
...
6. **多模型群聊**：多版本 Claude / GPT(codex) / Gemini(Antigravity `agy`)，桥接 + 防 loop + 各自记忆与人设。
...
[ 浏览器/手机 ]  --WSS/HTTPS-->  [ Cloudflare Edge ]  --tunnel-->  [ cloudflared ]
   ...
   │     ├─ codex runner（GPT）/ agy runner（Gemini）                     │
   │     └─ 静态资源自托管（/static）                                      │
...
| Gemini（小Z） | Antigravity `agy` 子进程 | Antigravity Google OAuth | 免费 Pro tier，可切 Gemini Pro / Claude 4.6 / GPT-OSS |
...
| `SANDBOX_DIR` | codex/agy 子进程的空工作目录 |
...
1. 装各模型 CLI（claude；可选 codex、Antigravity `agy`），各自完成登录。
