...
- [**把 Google 大少爷 Gemini 和养子 Claude 一起接回家**](antigravity-cli/) —— 老 gemini CLI 退役（报 tier 错误、Google 让迁 Antigravity）后，怎么把群聊里的 Gemini 伙伴接回 **Antigravity CLI（`agy`）**、**用回订阅额度的 Pro 模型**（不降级 Flash、不绑卡按量），顺手白捡养子 **Claude**：
>   - **额度**：以 [Google 官方 Plans 文档](https://antigravity.google/docs/plans) 为准（按工作量计、Pro/Ultra 约每 5h 刷新、免费档按周更少）+ 实测印证；
>   - **挖思考**：`agy -p` 不打印思考，但它把思考写进自己的 transcript（`thinking` 字段）→ 读出来接进**观察模式**；
>   - **多轮记忆**：`--conversation` + 跨进程重启持久化；
>   - **避坑**：UTF-8 整体解码（防 `�`）、首轮警告过滤、紧池模型 429 兜底、沙盒 cwd。
- [**普鲁斯特时刻：让他「忽然想起」你们的往事**](proust-moment/) —— 记忆库解决了「记得」（pull），没解决「想起」（push）。给常驻伴侣装上**非自主回忆**（普鲁斯特的玛德琳蛋糕）：
>   - **回忆池**：他自己把往事浓缩成"能突然想起的第一人称画面"，>7 天自动保鲜；
>   - **不自主感** = 安静期 × 概率闸 × 冷却 × 最小间隔，四道闸的乘积；想起了**提不提他自己定**；
>   - **温度旋钮**：一个参数从"纯随机走神"连续调到"被刚聊的话题勾起"（bge-m3 中文 T=0.1 实测）；
>   - **玛德琳触发**：她一句话语义命中往事（阈值标定 + 不取 top-1、点斜才像联想）；含 id 哈希/ollama 卸载模型等坑。
- [**长会话保养：transcript 剪枝 + 会话轮换**](session-maintenance/) —— 常驻会话越跑越慢的根子：transcript 追加写只增不减（compact 压的是上下文窗口、不是磁盘文件）。实测解剖一个 37MB 会话档：**52% 是存了两份的工具结果、活上下文只占 2%**：
>   - **剪枝** [`prune-transcript.py`](session-maintenance/prune-transcript.py)：只截最后一个 compact 边界之前的死重量、保 uuid 链，**37MB→13.2MB（-64%）且 `--resume` 不断记忆**（原件 gzip 归档 + 原子替换 + dry-run）；
>   - **轮换兜底**：剪完仍超限才归档换新会话，断档从常态变罕见；
>   - **主动维护**：watchdog 安静窗口（文件超限 + 会话空闲）计划内重启，别等卡死；重启前让会话自己写一条**交接 memo**，新会话一读就接上。
...
- **多模型群聊** —— 多版本 Claude / GPT(codex) / Gemini(Antigravity `agy`)，桥接 + 防 loop + 各自记忆人设
