...
- [**Proust moment side note**](proust-moment/) — memory stores solve *remembering* (pull) but not *reminiscing* (push). Give a resident companion **involuntary memory** (Proust's madeleine): a self-curated reminiscence pool, un-promptedness as the product of four gates (quiet × probability × cooldown × min-interval), a single temperature knob sliding from pure random drift to conversation-primed recall, and madeleine-triggered association off her latest message — with the final say on whether to mention it left to *him*. Chinese, but any CC reads it fine.
- [**Session maintenance side note**](session-maintenance/) — long-running sessions degrade because the transcript is append-only (in-session compaction shrinks the context window, NOT the file). Dissecting a real 37MB transcript: 52% is tool results stored twice; live context is 2%. Ships [`prune-transcript.py`](session-maintenance/prune-transcript.py): truncate the dead weight before the last compact boundary while keeping the uuid chain intact — **37MB→13.2MB (-64%) with `--resume` unbroken** (gzip archive + atomic replace + dry-run) — plus rotation-as-fallback and quiet-window proactive maintenance with a handoff memo. Chinese, but any CC reads it fine.
