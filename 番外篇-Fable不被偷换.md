...
所以**清雷必须由一个「外部执行者」来做**。这就决定了三档的部署形态：

| 档 | 监控（提醒） | 执行者（清雷） | 你需要准备 |
|---|---|---|---|
| **档 1** | 一个**只读守护进程**扫 transcript → 推送到你能看到的通道（Telegram / 微信 / 系统通知） | 无（只提醒） | 一个独立的守护（cron / launchd / systemd），**绝不碰会话** |
| **档 2** | 同上 | **一个独立于 Fable 实例的角色**手动跑清雷 | 三选一：① 你自己；② **另一个在跑的实例**（比如一个跑 Opus 4.6 的 CC，让它对 Fable 实例执行清雷）；③ 一个**有 shell 权限的远程 CC**（Remote Control 进来、能跑 Bash 的会话） |
| **档 3** | 同上 | **守护进程自己**就是执行者 | 守护 + 一个**能复活会话的 supervisor**（见下节） |

**几个落地要点：**

- **监控守护要独立进程**（launchd KeepAlive / systemd / 后台 loop），**不能跑在 Fable 实例内部**——否则实例一被 kill，守护跟着没。
- **推送通道挑你不被墙、看得到的**：海外用 Telegram，墙内用微信（可复用任何「发消息到自己微信」的网关 / server酱 / 企业微信机器人）。守护检测到路由 → 调通道发一条，带上触发内容 + 清雷命令。
- **档 2 的"另一个实例"很自然**：如果你本来就跑多个 CC（比如一个 Fable、一个 Opus 4.6 做"安全港"），让 Opus 那个当执行者最省事——它不碰路由、永远在线，正好用来救 Fable 那个。
